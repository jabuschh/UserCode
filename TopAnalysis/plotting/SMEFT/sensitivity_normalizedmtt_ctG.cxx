{
  ...
  for(int n=0; n<v_WilsonCoeff.size(); n++){
    ...
    */
    ...
    // EFT signals
    dir = "/nfs/dust/cms/user/jabuschh/PhD/ttbar/genstudy/CMSSW_9_4_6/src/UserCode/TopAnalysis/output/SMEFT/";
    ...
    for(int i=0; i<N_bins; i++){
      ...
      sens_min2p0[i] = h_ditopmass_min0p1 ->GetBinContent(i);
      sens_min1p0[i] = h_ditopmass_min0p05->GetBinContent(i);
      sens_1p0[i]    = h_ditopmass_0p05   ->GetBinContent(i);
      sens_2p0[i]    = h_ditopmass_0p1    ->GetBinContent(i);
      sens_3p0[i]    = h_ditopmass_0p15   ->GetBinContent(i);
      sens_4p0[i]    = h_ditopmass_0p2    ->GetBinContent(i);
      sens_5p0[i]    = h_ditopmass_0p25   ->GetBinContent(i);
      sens_err_min5p0[i] = h_ditopmass_min0p25->GetBinError(i);
      sens_err_min4p0[i] = h_ditopmass_min0p2 ->GetBinError(i);
      sens_err_min3p0[i] = h_ditopmass_min0p15->GetBinError(i);
      sens_err_min2p0[i] = h_ditopmass_min0p1 ->GetBinError(i);
      sens_err_min1p0[i] = h_ditopmass_min0p05->GetBinError(i);
      sens_err_1p0[i]    = h_ditopmass_0p05   ->GetBinError(i);
      sens_err_2p0[i]    = h_ditopmass_0p1    ->GetBinError(i);
    ...
    gr_min5p0->GetYaxis()->SetRangeUser(-4.1,4.1);
    ...
    legend->AddEntry(gr_min1p0, WC_name + " = -0.05","l");
    legend->AddEntry(gr_min2p0, WC_name + " = -0.10","l");
    legend->AddEntry(gr_min3p0, WC_name + " = -0.15","l");
    legend->AddEntry(gr_min4p0, WC_name + " = -0.20","l");
    legend->AddEntry(gr_min5p0, WC_name + " = -0.25","l");
    legend->AddEntry(gr_1p0   , WC_name + " = +0.05","l");
    legend->AddEntry(gr_2p0   , WC_name + " = +0.10","l");
    legend->AddEntry(gr_3p0   , WC_name + " = +0.15","l");
    legend->AddEntry(gr_4p0   , WC_name + " = +0.20","l");
    legend->AddEntry(gr_5p0   , WC_name + " = +0.25","l");
    ...
    break;
    c1->SaveAs("/nfs/dust/cms/user/jabuschh/PhD/ttbar/genstudy/CMSSW_9_4_6/src/UserCode/TopAnalysis/plots/SMEFT/sensitivity_normalizedmtt_" + WilsonCoeff + ".pdf");
    // break;
    c1->Close();
  }
}
